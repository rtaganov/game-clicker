...
export class Game {
  ...
  resetAttempt(){ const rig=this.getRig(); this.currentHeight=0; this.bestHeightAttempt=0; this.currentHeat=0; this.currentDurability=rig.durabilityMax*(1+this.mod('durability')); this.holdTime=0; this.attemptMoney=0; this.activeClickBoost=0; this.state='idle'; }
  ...
  buyRig(id){ const r=rigs.find(x=>x.id===id); if(this.ownedRigs.has(id)){ this.selectedRig=id; this.resetAttempt(); return;} if(this.money>=r.cost){this.money-=r.cost;this.ownedRigs.add(id);this.selectedRig=id;this.resetAttempt();} }
  buyMechanism(id){ const m=mechanisms.find(x=>x.id===id); if(this.ownedMechanisms.has(id)){ this.selectedMechanism=id; this.resetAttempt(); return;} if(this.money>=m.cost){this.money-=m.cost;this.ownedMechanisms.add(id);this.selectedMechanism=id;this.resetAttempt();} }
